...
export default function EditDestination() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(API_REQUESTS.editDestination(String(destinationId)), {
        method: 'PUT',
        body: formData,
        headers: {
      ...
      if(response.ok && data) {
        toast({
          title: `${data.data.name} updated successfully!`,
        });
        refresh();
  ...
  async function fetchDestinationDetails() {
    const response = await fetch(API_REQUESTS.destinationDetails(String(destinationId)), {
      cache: "no-store",
    });
    const data = await response.json();
